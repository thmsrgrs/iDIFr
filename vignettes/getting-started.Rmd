...
| `"RF"` | Random Forest structural-change test — non-parametric, detects intersectional instability |
| `"ID"` | Intersectional Decomposition — decomposes DIF into main effects, two-way, and intersection |
...
| RF | Std. score difference | Negligible: <.20 · Moderate: .20–.50 · Large: ≥.50 |

---

## Intersectional Contrast Analysis (ICA)

`ica()` goes one step further than a single `idifr()` call. It runs one
analysis per demographic variable (single-variable) *and* one intersectional
analysis, then classifies each item by comparing where it was flagged.
...
print(ica_res)   # classification table
tidy(ica_res)    # flat data frame
```
